[[NodeJS]] is by default single threaded, only one thread runs NodeJS code, however, many native APIs it uses are executed in different [[Thread]]s ([[Libuv]] thread pool).
