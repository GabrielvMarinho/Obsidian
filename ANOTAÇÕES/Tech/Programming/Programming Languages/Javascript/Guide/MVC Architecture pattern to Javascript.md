...
3. the routes dictates which [[URL]] is for each controller
4. the controller requests a serie of DTOs from the service layer
5. the service layer sends a [[Sql]] command to repository
...
good to pay attention to the fact that every thing related to the business logic is in the service layer, so the value of the [[Sql]] command is defined in the service layer, aswell as how the DTO is going to be turned into a model.
## /CONTROLLERS
#### userController.js
